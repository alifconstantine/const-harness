...
- `packages/*` (except agent-loop's own tests and examples/) must not import `@const-ai/agent-loop`.
- No cross-package deep imports (`@const-ai/*/src/...` paths) — public entry points only.
