...
| 包 | 路径 | ctx 面 | 本期 |
| --- | --- | --- | --- |
| `@const-ai/storage` | `packages/storage/storage/` | `ctx.storage`（枢纽） | ✓ |
| `@const-ai/storage-json` | `packages/storage/storage-json/` | 注册后端 `json` | ✓ |
| `@const-ai/storage-sqlite` | `packages/storage/storage-sqlite/` | 注册后端 `sqlite` | ✓ |
| `@const-ai/storage-domain` | `packages/storage/storage-domain/` | 挂载 `ctx.storage.domain` | ✓ |
