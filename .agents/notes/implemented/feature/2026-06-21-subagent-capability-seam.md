...
| Package | Role |
|---|---|
| `@const-ai/subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@const-ai/subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@const-ai/subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@const-ai/subagent-acp` | implementation: an ACP client driving a configured child process |
| `@const-ai/subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@const-ai/subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@const-ai/tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
