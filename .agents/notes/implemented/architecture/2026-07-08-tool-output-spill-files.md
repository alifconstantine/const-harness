...
| Package | Role |
|---|---|
| `@const-ai/spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@const-ai/spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@const-ai/spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@const-ai/spill-local'

- id: spill-policy
  name: '@const-ai/spill-policy'
  config:
...
- `@const-ai/output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@const-ai/spill` owns saving final text and returning a locator plus retrieval hint.
- `@const-ai/spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
