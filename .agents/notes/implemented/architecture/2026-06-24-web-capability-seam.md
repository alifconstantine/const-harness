...
1. `@const-ai/web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@const-ai/web-search-exa`, `@const-ai/web-search-perplexity`, `@const-ai/web-search-deepseek`, and `@const-ai/web-fetch-http`.
3. `@const-ai/tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@const-ai/tool-web  --depends on-->  @const-ai/web  <--depends on--  @const-ai/web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @const-ai/web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @const-ai/web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @const-ai/web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@const-ai/web-search-exa"] -->|registerSearchProvider| web["@const-ai/web / ctx.web"]
  perplexity["@const-ai/web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@const-ai/web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@const-ai/web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@const-ai/tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@const-ai/web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@const-ai/tool-web` depends on `@const-ai/web`, `@const-ai/tools`, `@const-ai/system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@const-ai/web'
...
- id: web-search-exa
  name: '@const-ai/web-search-exa'

- id: web-search-perplexity
  name: '@const-ai/web-search-perplexity'

- id: web-search-deepseek
  name: '@const-ai/web-search-deepseek'

- id: web-fetch-http
  name: '@const-ai/web-fetch-http'
