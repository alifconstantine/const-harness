...
const RELEASE_ENTRY_PACKAGE = '@const-ai/cli'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@const-ai/')) {
        throw new Error(`${manifestPath} must name an @const-ai package`)
      }
      if (name === '@const-ai/root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@const-ai/cli/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@const-ai/') || name === '@const-ai/root')) {
