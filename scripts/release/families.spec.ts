...
describe('release families', () => {
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@const-ai/cli')
    const cordis = { ...member('vendor/cordis', '@const-ai/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@const-ai/cli'), { ...member('apps/web', '@const-ai/web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@const-ai/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@const-ai/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@const-ai/consumer', { dependencies: { '@const-ai/library': 'workspace:^' } }),
      member('packages/a/library', '@const-ai/library'),
      member('packages/a/zebra', '@const-ai/zebra'),
    ]
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@const-ai/left', { dependencies: { '@const-ai/right': 'workspace:^' } }),
      member('packages/a/right', '@const-ai/right', { dependencies: { '@const-ai/left': 'workspace:^' } }),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@const-ai/library')
    const vendored = member('vendor/cordis', '@const-ai/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@const-ai/cli', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@const-ai/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@const-ai/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@const-ai/library', { files: ['lib/index.js'] }),
