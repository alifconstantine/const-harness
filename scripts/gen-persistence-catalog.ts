...
const SESSION_PACKAGE = '@const-ai/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@const-ai/session/types'
 ...
 * top-level declaration (in `@const-ai/session`) and any declaration
 * merge inside a `declare module '@const-ai/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @const-ai/session/known-event-types',
    ' */',
    '',
    '/**',
