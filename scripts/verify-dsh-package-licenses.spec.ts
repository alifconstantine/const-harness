...
describe('DSH package license gate', () => {
  it('checks root, unhyphenated CLI, and dsh-prefixed package names while ignoring other families', () => {
    ...
    writeManifest(root, 'apps/cli/package.json', { name: '@const-ai/cli', license: 'MIT' })
    ...
    expect(inspectDshPackageLicenses(root)).toEqual({
      packageCount: 2,
      failures: [
        'packages/core/agent/package.json: @const-ai/agent must declare "license": "MIT"; found "BSD-3-Clause".',
      ],
    })
  })
  ...
  it('rejects a missing license declaration', () => {
    const root = createWorkspace()
    writeManifest(root, 'packages/core/agent/package.json', { name: '@const-ai/agent' })

    expect(inspectDshPackageLicenses(root).failures).toEqual([
      'packages/core/agent/package.json: @const-ai/agent must declare "license": "MIT"; found undefined.',
    ])
  })
})
