...
const CHOOSER_PACKAGE = '@const-ai/host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@const-ai/host-directory-picker-native',
  '@const-ai/host-directory-picker-browse',
  '@const-ai/client-ui-directory-picker-browse',
  '@const-ai/client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@const-ai/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@const-ai/cordis-plugin-include') return
