...
const MERGE_HEAD = /declare module ['"](?:@const-ai\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@const-ai/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@const-ai/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@const-ai/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@const-ai/cordis'` Context merge
...
/**
 * Every event name a `declare module '@const-ai/cordis'` Events merge declares in one
