...
import { createScope } from '@const-ai/scope'
...
import GoalService from '@const-ai/goal'
...
import { PwshLocalExecutor } from '@const-ai/pwsh-local'
import LocalSubprocessRuntime from '@const-ai/subprocess-local'
import LocalFileSystem from '@const-ai/fs-local'
import { AttachmentStore } from '@const-ai/attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@const-ai/attachment'
import UserQuestionService from '@const-ai/user-questions'
import PlanModeController from '@const-ai/plan-mode'
import WebRuntime from '@const-ai/web'
import * as WebSearchExa from '@const-ai/web-search-exa'
import * as WebFetchLocal from '@const-ai/web-fetch-http'
import SubagentRuntime from '@const-ai/subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@const-ai/subagent'
import * as ToolSubagentControl from '@const-ai/tool-subagent-control'
import * as ToolSubagentListAgents from '@const-ai/tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@const-ai/tool-subagent-report'
import SkillRegistry from '@const-ai/skill'
import * as SkillFileSystem from '@const-ai/skill-filesystem'
import LocalJobRegistry from '@const-ai/jobs-local'
import * as ToolAskUser from '@const-ai/tool-ask-user'
import * as ToolBash from '@const-ai/tool-bash'
import * as ToolPwsh from '@const-ai/tool-pwsh'
import * as ToolBashPersistent from '@const-ai/tool-bash-persistent'
import CordisHostRunner from '@const-ai/cordis-host-runner'
import * as ToolCordis from '@const-ai/tool-cordis'
import * as ToolFs from '@const-ai/tool-fs'
import * as ToolFsSearch from '@const-ai/tool-fs-search'
import * as ToolStrReplaceEditor from '@const-ai/tool-str-replace-editor'
import TerminalSessionService from '@const-ai/terminal'
import * as ToolPty from '@const-ai/tool-terminal'
import * as ToolGoal from '@const-ai/tool-goal'
import * as ToolSchedule from '@const-ai/schedule'
import Lsp from '@const-ai/lsp'
import * as ToolLsp from '@const-ai/tool-lsp'
import * as ToolSkill from '@const-ai/tool-skill'
import * as ToolSessionQuery from '@const-ai/tool-session-query'
import * as ToolTasks from '@const-ai/tool-jobs'
import * as ToolTodo from '@const-ai/tool-todo'
import * as ToolSubagent from '@const-ai/tool-subagent'
import * as ToolWeb from '@const-ai/tool-web'
import VmWorkflowEngine from '@const-ai/workflow-worker-thread'
import * as ToolRalph from '@const-ai/tool-ralph'
import * as ToolWorkflow from '@const-ai/tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@const-ai/tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@const-ai/tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@const-ai/pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@const-ai/shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@const-ai/tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@const-ai/cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@const-ai/tool-bash-persistent',
  ...
  {
    pkg: '@const-ai/tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@const-ai/fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@const-ai/tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@const-ai/lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@const-ai/tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@const-ai/tool-session-query',
  ...
  {
    pkg: '@const-ai/tool-subagent-control',
  ...
  {
    pkg: '@const-ai/tool-subagent-report',
