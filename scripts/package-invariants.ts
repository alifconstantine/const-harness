...
function checkManifest(
  ...
  if (owner.packageName === '@const-ai/invariants') return
  if (manifest.peerDependencies?.['@const-ai/invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@const-ai/invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@const-ai/invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@const-ai/invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@const-ai/invariants'
