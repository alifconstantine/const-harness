...
flowchart LR
  ...
  svc_snapshot["ctx.snapshot<br/>Workspace file snapshot and rollback"]
  ...
  svc_snapshot --> pkg_host_apiproxy
...
| `ctx.snapshot` | `core` | [`fs-snapshot`](../packages/fs/fs-snapshot) | - | [`host-apiproxy`](../packages/host/apiproxy) | - | Captures immutable git tree snapshots in isolated state storage for turn-level rollback and diff inspection. |
