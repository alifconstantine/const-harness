 ...
 * REAL `@const-ai/sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@const-ai/acp-snapshot'
import { resolveExampleLaunch } from '@const-ai/loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@const-ai/sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@const-ai/system-prompt') return []
