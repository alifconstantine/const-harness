...
import type { StateDotState } from '@const-ai/client-ui-primitives'
 ...
 * @param props.onDelete - delete a session permanently (opens confirmation dialog).
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, drag, flat = false, t }: {
  ...
  /** Open the browser-owned delete-confirmation dialog (row menu action). */
  ...
  const sessionMenuItems = [
    ...
    ...onDelete !== undefined ? [{ id: 'delete', label: t('delete.session') || t('delete'), icon: <IconTrashOutline16 />, danger: true }] : [],
  ]
