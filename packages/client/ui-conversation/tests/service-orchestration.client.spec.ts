...
import { makeTranslate, SlotTestRuntime } from '@const-ai/client-test-runtime'
import type { QueuedMessage, SessionFace } from '@const-ai/client-runtime/client'
...
async function bench(readAttachment?: SessionFace['readAttachment']) {
  ...
  const rollbackTurn = vi.fn(() => Promise.resolve({ userPrompt: 'prompt', restoredFiles: [] }))
  await runtime.sessions.add({
    id: 's1',
    session: { prompt, updateQueue, cancel, loadOlder, rollbackTurn, ...(readAttachment === undefined ? {} : { readAttachment }) },
  })
