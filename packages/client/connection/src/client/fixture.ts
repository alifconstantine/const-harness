...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@const-ai/llm/message'
import { CallId } from '@const-ai/llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type {
  ...
  SessionId,
  TodoItem,
} from '@const-ai/session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@const-ai/commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@const-ai/commands/types'
import { deriveEventMessage, foldSurface } from '@const-ai/session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@const-ai/host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@const-ai/tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@const-ai/tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@const-ai/tool-read'\n" }],
  ])
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      rollbackTurn: request => ok(request, {
        success: true,
        restoredFiles: [],
      }),
    },
    ...
    workspace: {
      ...
      unarchiveSession: (request) => {
        ...
        if (idx !== -1) {
          archivedSessionIds.splice(idx, 1)
          emitHost({ type: 'host/archived-sessions-changed', archivedSessionIds: [...archivedSessionIds] })
        }
        return ok(request, { archivedSessionIds: [...archivedSessionIds] })
      },
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.rollbackTurn': return this.api.sessions.rollbackTurn(request)
