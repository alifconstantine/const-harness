...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@const-ai/llm'
...
import * as providerPlugin from '@const-ai/session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@const-ai/session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@const-ai/session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@const-ai/session-title', SessionTitleService],
    ['@const-ai/session-title-first-prompt-llm', providerPlugin],
  ])
