...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@const-ai/tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@const-ai/tools'
import type { Agent } from '@const-ai/agent'
import { Session, SessionId } from '@const-ai/session'
import type { JsonValue, SessionEventMap } from '@const-ai/session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@const-ai/scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
