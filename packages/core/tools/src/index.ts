 ...
 * execution pipeline.
 * @module @const-ai/tools
 */

import { Context, Service } from '@const-ai/cordis'
import z from '@const-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@const-ai/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@const-ai/scope'
import type { CallId, ContentBlock, ToolSchema } from '@const-ai/llm'
import { assertNever, deepFreeze, HarnessError } from '@const-ai/llm'
import type { Agent } from '@const-ai/agent'
import { snapshotJsonValue } from '@const-ai/session'
import type { JsonValue, UserMessage } from '@const-ai/session'
import type { ToolProviderResult } from '@const-ai/system-prompt'
import type { CodeRuntime } from '@const-ai/code-runtime'
...
export type { JsonValue } from '@const-ai/session'
...
// lives in its own UI-facing module; re-export it so `@const-ai/tools`
...
declare module '@const-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@const-ai/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@const-ai/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@const-ai/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@const-ai/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@const-ai/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @const-ai/code-runtime-worker-thread) or set tools mode to "native"`)
    }
