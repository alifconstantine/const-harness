 ...
 *
 * @module @const-ai/system-prompt
 */

import { Context, Service } from '@const-ai/cordis'
import z from '@const-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@const-ai/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@const-ai/scope'
import type { ContextSnapshotSection, ToolSchema } from '@const-ai/llm'
...
declare module '@const-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@const-ai/scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Const Harness.',
      })
    }
    this.section({
