...
| Package | Role |
|---|---|
| `@const-ai/shell` (this) | Service Definition: abstract service + vocabulary types |
| `@const-ai/bash-local` | Service Provider: local subprocesses |
| `@const-ai/bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@const-ai/tool-bash` | the model-facing tool schemas over `ctx.shell` |
