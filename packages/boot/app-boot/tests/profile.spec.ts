...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@const-ai/base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @const-ai/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@const-ai/base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    ...
    initProfile(custom, [
      '@const-ai/base', '@const-ai/web-app', '@const-ai/headless', 'custom-bundle',
    ])
