...
import Storage from '@const-ai/storage'
import * as StorageDomain from '@const-ai/storage-domain'
import * as StorageJson from '@const-ai/storage-json'
import { remoteMethods } from '@const-ai/typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@const-ai/session-persistence-jsonl', JsonlSessionPersistence],
    ['@const-ai/storage', Storage],
    ['@const-ai/storage-json', StorageJson],
    ['@const-ai/storage-domain', StorageDomain],
    ['@const-ai/message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@const-ai/session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@const-ai/storage'",
      "- name: '@const-ai/storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@const-ai/storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@const-ai/message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
