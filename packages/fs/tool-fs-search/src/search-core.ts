 ...
 *
 * @module @const-ai/tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@const-ai/cordis'
import { HarnessError } from '@const-ai/llm'
import { ItemRetainer, TextRetainer } from '@const-ai/output-retention'
import type { RetainedItems } from '@const-ai/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@const-ai/subprocess'
import type { SaveTextSpill, SpillRef } from '@const-ai/spill'
 ...
 * `@const-ai/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@const-ai/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@const-ai/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@const-ai/tool-fs-search/presentation} `grepSearchMeta`)
