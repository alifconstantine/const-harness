...
```
@const-ai/cordis-plugin-timer  timer service (writes nothing to stdout)
@const-ai/llm              abstract LLM service + content-block vocabulary
@const-ai/session          event-sourced session log + store
@const-ai/session-title    log-backed title service + deterministic fallback
@const-ai/system-prompt    prompt-section + tool-schema assembly
@const-ai/tools            registry + guarded pre/around/post/final-result pipeline
@const-ai/skill            skill provider registry
@const-ai/skill-filesystem      local filesystem skill provider
@const-ai/agent            agent registry + initiator scope + agent/* events
@const-ai/goal             optional persisted same-session goal domain
@const-ai/tool-goal        optional model-facing goal controls
@const-ai/goal-round-driver     optional same-session goal-round driver
@const-ai/llm-retry        provider-routed request retry policy
@const-ai/jobs-local      generic background-job registry
@const-ai/invariants       configurable invariant registry service
@const-ai/session/invariant
@const-ai/agent/invariant
@const-ai/scope/invariant
@const-ai/agent-loop/invariant
                                  package-owned relational checks
@const-ai/tool-bash        the model-facing bash schema (unless toolBash=false)
@const-ai/agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@const-ai/tool-skill       session-prefix skill catalog + model-facing loader schema
@const-ai/tool-jobs       job_output/job_list/job_kill schemas + completion notices
@const-ai/agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@const-ai/agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@const-ai/'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
