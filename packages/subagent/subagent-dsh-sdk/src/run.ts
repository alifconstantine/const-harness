 ...
 * runtime over stdio JSON-RPC through `@const-ai/sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@const-ai/subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @const-ai/subagent-dsh-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { DeepSeekHarness, type HarnessNotification } from '@const-ai/sdk-client'
import type { ContentBlock } from '@const-ai/llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@const-ai/session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@const-ai/subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@const-ai/subagent'
import { scrubbedParentEnv } from '@const-ai/subprocess'
