 ...
 *
 * @module @const-ai/subagent
 */

import type { Context } from '@const-ai/cordis'
import type { Session, SessionEvent, SessionHeader, SessionId } from '@const-ai/session'
import type { SessionPersistence } from '@const-ai/session-persistence'
import type { SessionProjectionRegistry } from '@const-ai/session-projection'
import type { SessionProjectionCache } from '@const-ai/session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @const-ai/session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @const-ai/session)',
