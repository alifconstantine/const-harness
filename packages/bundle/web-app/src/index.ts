/**
 * @const-ai/web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@const-ai/app-boot'
import * as FrontendStatic from '@const-ai/host-frontend-static'
import type {} from '@const-ai/cordis-plugin-loader'
import type {} from '@const-ai/host-webserver'
import type {} from '@const-ai/system-prompt'
import type {} from '@const-ai/shell-env'
...
/** Model-visible orientation and acceptance boundary for sessions created through `const web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the Const Harness Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only const web injects window.__DSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@const-ai/web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the Const Harness Web GUI serving this session.' },
        },
  ...
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`const web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
