...
import LocalCredentialProvider from '@const-ai/credentials-local'
import { settingsNamespace } from '@const-ai/settings'
import FileSettingsProvider from '@const-ai/settings-file'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@const-ai/settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@const-ai/credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@const-ai/settings-file', FileSettingsProvider],
    ['@const-ai/credentials-local', LocalCredentialProvider],
    ['@const-ai/llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
