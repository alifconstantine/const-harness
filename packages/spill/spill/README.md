...
| Package | Role |
|---|---|
| `@const-ai/spill` (this) | Service Definition: abstract service + vocabulary types |
| `@const-ai/spill-local` | Service Provider: private session-scoped files on the host filesystem |
| `@const-ai/spill-policy` | Consumer: the tool-result policy that spills oversized final results |
...
Storage is grouped by the request's `owner` session as a save-time namespace; the backend chooses its own private representation and may derive names from — never trust as a path — the caller's `suggestedName`. The seam owns storage only: NO retention policy (that is [`@const-ai/output-retention`](../../util/output-retention)), NO tool-result replacement (that is `@const-ai/spill-policy`), NO retrieval/search API (the backend's `retrievalHint` tells the model what to do with the locator).
