...
import {
  ...
  sessionRollbackTurnValueSchema,
...
export interface IApiClient {
  sessions: {
    ...
    rollbackTurn(payload: RequestPayload<'session.rollbackTurn'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.rollbackTurn'>>>
  }
  subagents: {
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.rollbackTurn': sessionRollbackTurnValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    rollbackTurn: (payload, signal) => this.callUnary('session.rollbackTurn', payload, signal),
  }
