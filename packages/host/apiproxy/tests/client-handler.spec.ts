...
import type { ApiProxy, GoalRef, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@const-ai/host-apiproxy'
import { InProcessApiClient, RpcId, toFetchHandler } from '@const-ai/host-apiproxy'
...
function scriptedApi(overrides: {
  ...
  return {
    ...
    automations: {
      ...
      create: r => ok(r, {
        item: {
          ...
          schedule: r.payload.schedule,
          permissionPreset: r.payload.permissionPreset ?? 'workspace-write',
          enabled: true,
    ...
    design: {
      ...
      templateDetail: err,
      craftGuidelines: r => ok(r, { guidelines: [] }),
      craftGuideline: err,
