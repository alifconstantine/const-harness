...
import { TypertLookupFailure } from '@const-ai/typert-protocol'
import TypertRegistry from '@const-ai/typert-registry'
import { createUserMessage, MessageId } from '@const-ai/llm'
import type { Agent } from '@const-ai/agent'
import UserQuestionService from '@const-ai/user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@const-ai/session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@const-ai/session-persistence'
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@const-ai/session').Session
