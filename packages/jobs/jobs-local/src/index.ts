 ...
 * @module @const-ai/jobs-local
 */

import { Context } from '@const-ai/cordis'
import z from '@const-ai/schemastery'
import type { Agent } from '@const-ai/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@const-ai/scope'
import type { ScopeLayer } from '@const-ai/scope'
import { deadline, timeoutOf } from '@const-ai/timeout'
import { JobRegistry, JobId } from '@const-ai/jobs'
import type {
 ...
 * `@const-ai/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @const-ai/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @const-ai/agent)')
    }
