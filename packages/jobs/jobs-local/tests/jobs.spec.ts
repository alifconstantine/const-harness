...
import { bindScopeParent, createScope, scopeOf } from '@const-ai/scope'
import type { ScopeKey } from '@const-ai/scope'
import { JobId } from '@const-ai/jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@const-ai/jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @const-ai/tool-jobs in its composition)')
  })
