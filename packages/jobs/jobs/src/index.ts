 ...
 * process-local registry lives in `@const-ai/jobs-local`.
 * @module @const-ai/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@const-ai/jobs is the abstract job registry seam; load an implementation such as @const-ai/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
