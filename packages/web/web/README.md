...
| Package | Role |
|---|---|
| `@const-ai/web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@const-ai/web-search-exa` | Search provider: Exa |
| `@const-ai/web-search-perplexity` | Search provider: Perplexity |
| `@const-ai/web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@const-ai/tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
