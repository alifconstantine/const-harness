...
} from '@const-ai/node-addon-landlock-run'
import { Context } from '@const-ai/cordis'
import z from '@const-ai/schemastery'
import { assertNever } from '@const-ai/llm'
import { SandboxProvider, SandboxUnavailableError } from '@const-ai/sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@const-ai/sandbox'
import type { SessionId } from '@const-ai/session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@const-ai/sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@const-ai/sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@const-ai/sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@const-ai/sandbox-windows-acl/src/runner.ts'))
