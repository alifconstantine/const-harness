...
Implementations: [`@const-ai/sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@const-ai/bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).
