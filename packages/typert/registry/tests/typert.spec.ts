...
} from '@const-ai/typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@const-ai/typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@const-ai/tools', 'ToolInput')).toBe('@const-ai/tools#ToolInput')
    expect(typertPackageKey('@const-ai/tools', 'host')).toBe('@const-ai/tools#host')
    expect(ctx.typert.get('@const-ai/tools#ToolInput')).toMatchObject({
      package: '@const-ai/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@const-ai/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@const-ai/tools', 'host')).toMatchObject({
      key: '@const-ai/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@const-ai/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@const-ai/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@const-ai/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@const-ai/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@const-ai/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@const-ai/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@const-ai/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@const-ai/tools#ToolInput')).not.toBe(projected)
  })
