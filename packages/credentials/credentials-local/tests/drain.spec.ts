...
vi.mock('@const-ai/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@const-ai/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@const-ai/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
