...
/**
 * Keyless built-artifact smoke: plain Node imports `@const-ai/lsp` and
 * `@const-ai/lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@const-ai/cordis')
      const { default: Lsp } = await import('@const-ai/lsp')
      const LspLocal = await import('@const-ai/lsp-stdio')
      const { default: LocalFileSystem } = await import('@const-ai/fs-local')
      const { default: LocalSubprocessRuntime } = await import('@const-ai/subprocess-local')
