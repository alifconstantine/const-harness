...
| Package | Role |
|---|---|
| `@const-ai/lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@const-ai/lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@const-ai/tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
