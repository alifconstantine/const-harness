...
packages/    @const-ai/<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @const-ai/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@const-ai/<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@const-ai/cordis` is a peerDependency (+ dev) of every harness package.
