...
Prebuilt `bin/landlock-run` Landlock launcher for linux-x64 — a static musl binary compiled natively (no cross toolchain) from the C source shipped in [`@const-ai/node-addon-landlock-run`](https://www.npmjs.com/package/@const-ai/node-addon-landlock-run). npm's `os`/`cpu` fields select this package at install time; the entry package resolves it to a file path — it ships no JavaScript and is never imported.
...
Sibling: `@const-ai/node-addon-landlock-run-linux-arm64`.
