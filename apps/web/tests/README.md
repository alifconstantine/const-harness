...
## Do not import `@const-ai/client-*` here
...
`@const-ai/goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@const-ai/client-web` and the boot-manifest
type from `@const-ai/client-modules/client`: booting the real shell is what
...
`@const-ai/client-runtime/client` because `client/runtime` is reachable
