...
import Include, { type PatchOptions } from '@const-ai/cordis-plugin-include'
import Group from '@const-ai/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@const-ai/acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@const-ai/app-boot'
import { dshHomePath } from '@const-ai/home-paths'
import { settingsNamespace } from '@const-ai/settings'
import { LlmAdapter } from '@const-ai/llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@const-ai/llm'
import type { ReplayHandle } from '@const-ai/llm-replay'
import { installLlmReplay, parseSessionLog } from '@const-ai/llm-replay'
import SessionStore, {
  packChunkRuns,
...
import type {} from '@const-ai/agent'
import { provideCmdline } from '@const-ai/cmdline'
...
// } from '@const-ai/client-ui-settings-models'
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // (apps/web IS @const-ai/web-frontend); only the URL line is silenced.
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@const-ai/host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@const-ai/client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@const-ai/tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    // `@const-ai/cordis-plugin-group` by name.
