...
import { deriveReplayScript, parseSessionLog } from '@const-ai/llm-replay'
import type { ReplayOverrideDoc } from '@const-ai/llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  it.skipIf(MODE === 'record')('cancels a hung stream deterministically via the readyFile marker', async () => {
    ...
    await expect.poll(
      () => page.getByRole('status').filter({ hasText: /Deep diving\.\.\.|Working…|正在处理…/ }).isVisible(),
      { timeout: 10_000 },
    ).toBe(true)
