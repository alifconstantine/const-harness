...
import { boot, loadOverlayPatches } from '@const-ai/app-boot'
import SystemPrompt from '@const-ai/system-prompt'
import ToolRuntime from '@const-ai/tools'
import * as McpClient from '@const-ai/mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@const-ai/mcp-client')
