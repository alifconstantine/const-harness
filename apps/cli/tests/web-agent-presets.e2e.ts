...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@const-ai/app-boot'
import { provideCmdline } from '@const-ai/cmdline'
import { SessionId } from '@const-ai/session'
import type { Agent } from '@const-ai/agent'
import type { PatchOptions } from '@const-ai/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@const-ai/settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@const-ai/agent-presets'
import { applyChildComposition, childSessionMeta } from '@const-ai/subagent'
import { CallId } from '@const-ai/llm'
import type {} from '@const-ai/compaction-basic'
import type {} from '@const-ai/skill'
import type {} from '@const-ai/tools'
// Type-only: resolves `ctx.get('sessionProjections')` and `ctx.get('tokenMeter')`.
import type {} from '@const-ai/session-projection'
import type {} from '@const-ai/token-meter'
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@const-ai/subagent-codex' },
        { id: 'subagent-claude-code', name: '@const-ai/subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
