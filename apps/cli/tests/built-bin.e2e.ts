...
import { startMockLlmServer } from '@const-ai/llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@const-ai/base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@const-ai/cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@const-ai/cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@const-ai/base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@const-ai/base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@const-ai/agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @const-ai/base')
      expect(stdout).toContain("name: '@const-ai/host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@const-ai/headless'")
      expect(stdout).not.toMatch(/name: '@const-ai\/host-/)
      expect(stdout).not.toContain("name: '@const-ai/web-app'")
      expect(stdout).not.toMatch(/name: '@const-ai\/client-/)
    }, 30_000)
