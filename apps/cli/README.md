...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@const-ai/base`, `@const-ai/web-app`, `@const-ai/headless`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
